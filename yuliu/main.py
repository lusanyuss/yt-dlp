...
def check_and_run(sub_directory, juji_num, split_time_min, is_clear_cache=False):
    video_template = '{}.mp4'
    # 生成所有视频文件的路径
    ...
    if missing_files:
        ...
        if len(videos) == juji_num:
            ...
            run_main(sub_directory=sub_directory, videos=videos, split_time_min=split_time_min, is_clear_cache=is_clear_cache)
        else:
...
if __name__ == "__main__":
    ...
    # check_and_run('报告妈咪 爹地是总裁', 1, 15, is_clear_cache=True)#有3家
    for video in videos:
        check_and_run(video, 1, 15, is_clear_cache=True)
    #
    # check_and_run('工业之光', 1, 15, is_clear_cache=True)#有1家
    # check_and_run('aa测试目录', 1, 0.1, is_clear_cache=True)
